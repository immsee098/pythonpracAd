...
# 2.변수 등에 할당 가능->데코레이터, 클로저 가능
# 3.함수 인수 전달 가능->함수 안에 함수 전달 가능sorted(len=key)
    #함수의 param으로 함수를 넣는다는 의미(자바는 이거 잘 안함)
# 4.함수 결과로 반환 가능 -> 함수 자체를 반환 가능

#함수를 객체 취급함. 그래서 클래스든 뭐든 다 객체취급함(Attribute)
...
print('EX1-3 -', type(factorial), type(A))  #모두 function이나 마찬가지
#집합-집합은 불가능하니 set으로 만들어서 차집합해버림(그럼 factorial메서드만 보임)
...
print('EX3-3 -', reduce(add, range(1,11))) # 누적 (+=같은 느낌)
...
#dir 찍었을 때 __call__있음 호출 가능한겨
#func() 형식으로 함수 호출하듯 호출할 수 있단 의미
...
class LottoGame:
    ...
    #해당 메서드를 오버로딩하여 작성해주면 함수처럼 리턴댐
    #자기가 지정해둔 메서드를 바로바로 함수처럼 부를수있음(func()같은 느낌)
    def __call__(self):
        return self.pick()
...
# 객체 직접 호출
print('EX4-4 -', callable(game))   #callable(str), callable(list) 등 뭘 넣어보면 저걸 호출할수있나 알려줌
#game에 __call__할 수 없었음 그치만 메서드 만들어줌
...
# 하나 이상의 인수가 할당된(채워진) 함수의 새 버전 반환
...
#d인수 고정
five = partial(mul, 5) #이후 five 호출할 때는 인수 하나만 받음 -> 두 개 넣음 에러남


# 고정 추가
six = partial(five, 6) #이미 five에서 5로 고정되어있어서 인수 하나를 쓰고 거기에 6나와서 다쓴셈
