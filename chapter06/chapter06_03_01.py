...
# Future 동시성
# 비동기 작업 실행
# 지연시간(Block) CPU 및 리소스 방비 방지 -> Network I/O 관련 직업 동시성 활동 권장
# 적합한 작업일 경우 순차 진행보다 압도적으로 성능 향상

# 실습 대상 3가지 경우

# 순차 실행
# concurrent.futures 방법1(ThreadPoolExexuter, ProcessPoolExecuter)
# concurrent.futures 방법2

# 파이썬은 글로벌 인터페이스 lock이 걸려있음
# 그래서 여러 스레드 작업해도 하나 실행중에는 다른거 멈춤
